...
Every plan is written for an implementation pass with **zero context**: it has not seen the advisor session, the audit, the other plans, or any prior conversation. Assume the implementer is competent at following explicit instructions and weak at filling gaps, recovering from ambiguity, or knowing when to stop.
...
2. **Verification gates** — every step ends with a command and its expected result. The implementer never has to *judge* whether it succeeded.
...
lands. Written so the implementer (and a human reviewer) understands the intent —
intent is what lets a correct judgment call happen when a detail is off.

## Current state

The facts the implementer needs, inlined — never "as discussed" or "see audit":
...
- Excerpts of the code as it exists today (short, with `file:line` markers),
  enough that the implementer can confirm it's looking at the right thing.
...
- Any documented vocabulary or design constraints the plan must honor, inlined
  ...
  the implementer should use in names and comments, the `DESIGN.md` tokens/components
...
implementation environment. Skip the section otherwise.)

- Skills to invoke if available, and for what:
...
Written once by the advisor after all plans, updated during implementation:

```markdown
# Implementation Plans

Generated by the improve skill on <date>. Execute in the order below unless
dependencies say otherwise. Each implementation pass: read the plan fully before starting,
